...
function refreshTrades(scope, HubTrades, StationTrades, PenuryTrades) {
    scope.hubTrades = angular.fromJson(HubTrades.get({station: scope.hub.id}));
    scope.stationTrades = angular.fromJson(StationTrades.get({station: scope.hub.id}));
    scope.penuryTrades = angular.fromJson(PenuryTrades.get({station: scope.hub.id}));
}
